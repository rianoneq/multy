...
class post_req_parser:
    ...
    def parse_shipovertheworld(self, session):
      s = session.get(url=self.parse_url, headers={'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:109.0) Gecko/20100101 Firefox/115.0'})
      soup = BeautifulSoup(s.text, 'html.parser')
      data = []
    ...
    def parse_sbx(self, session):
      ...
      for item in all_data:
        if item:
          ...
          if ('working' in status_and_exp):
            expired = 'more than a week' if 'more than a week' in status_and_exp else re.findall(r"\d\d\d\d-\d\d-\d\d", status_and_exp)[0] if len(re.findall(r"\d\d\d\d-\d\d-\d\d", status_and_exp))>0 else 'n/a'
            status = 'working'
          elif ('new courier' in status_and_exp):
            expired = re.findall(r"\d\d\d\d-\d\d-\d\d", status_and_exp)[0] if len(re.findall(r"\d\d\d\d-\d\d-\d\d", status_and_exp))>0 else 'n/a'
            status = 'new courier'
          else:
            for exp in status_and_exp.split():
              if '.' in exp :
                ...
                expired = 'n/a'
            expired = re.findall(r"\d\d\d\d-\d\d-\d\d", status_and_exp)[-1] if len(re.findall(r"\d\d\d\d-\d\d-\d\d", status_and_exp))>0 else 'n/a'
            status = 'not work'
    ...
    def log_in(self, login_name, password_name):
        ...
        if 'leonproff' in self.first_url:
          csrf = BeautifulSoup(session.get(self.first_url).text).find('meta', {'name': 'csrf-token'})['content']
          data = {
            '_csrf': csrf,
            'LoginForm[username]': 'luciano',
            'LoginForm[password]': 'V1valD1',
            'LoginForm[rememberMe]': [ '0', '1'],
            'login-button': '',
          }
          r = session.post('https://stuffer.leonproff.com/site/login', data=data)
        elif 'shipovertheworld' in self.first_url:
          data = {
            'login': self.login,
            'password': self.password,
          }
          r = session.post(self.first_url, headers=headers, data=data)
          import time
          time.sleep(4)
          r = session.get(self.parse_url)
          ...
          cookies = {
              'cf_clearance': 'dlP.12K8siErue0x4NUhCz8Rqb8vXS5sN0nSjE4fga8-1665598376-0-150',
          }
          proxies = {
    ...
    def parse(self):
        if 'gonzaproject.biz' in self.first_url:
          ...
          return couriers
        elif 'shipovertheworld.com' in self.first_url:
          session = self.log_in('LoginForm[username]', 'LoginForm[password]')
          couriers = self.parse_shipovertheworld(session=session)
          return couriers
        ...
        #тоже самое наверн
          ...
          return couriers
        elif 'leonproff.com' in self.first_url:
          session = self.log_in('LoginForm[username]', 'LoginForm[password]')
          couriers = self.parse_admwest(session=session)
          return couriers
...
#  service='desertgun').parse()
# print(len(data))

# data = post_req_parser(parse_url='https://api.tort.top/couriers/filter',
...
# data = post_req_parser(parse_url='https://shipovertheworld.com/stuffer/drops.php',
#  first_url='https://shipovertheworld.com/stuffer/login.php', 
#  login='lucianovivaldi', 
#  password='BVPmhE864Am', 
 
#  service='Service: USA дроп-сервис LEON [SEPTEMBER]').parse()
# print((data))
# data = post_req_parser(parse_url='http://stuffer.leonproff.com/drop/index',
#  first_url='http://stuffer.leonproff.com/site/login', 
#  login='luciano', 
#  password='V1valD1', 
